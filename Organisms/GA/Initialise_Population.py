...
def Check_Population_against_predation_operator(population, predation_operator):
	...
	"""
	import pdb
	# The population will be checked by the Fitness_Factor_Scheme for duplicates. Check_Initial_Population()
	# will delete any clusters that are repeats and present them as cluster_numbers_to_create
	...
	if not population.size - len(population) == len(clusters_to_create):
		...
		print('It is expected that the difference in the population compared to the maximum number that we '
			  'want should equal the number of entried in cluster_numbers_to_create')
	...
	if len(population) > population.size: 
		...
		# If any clusters need to be created to fit inline with the Fitness_Factor_Scheme,
		# repeat the process to get new randomised clusters (repeat while loop).
		...
		return clusters_to_create, removed_clusters_report  # Finished initialisation process, pool is complete
	else:
...
def Place_Already_Created_Clusters_In_Population(population, cluster_makeup, Minimisation_Function,
												 Initial_Energy_Function,
	...
	print('Checking to see if the user has any clusters in the folder ' +
	...
	# The folder nammed by the variable self.user_initialised_population_folder exists.
	...
	for cluster_name in os.listdir(population.user_initialised_population_folder):
		if cluster_name.startswith('.'):
			continue
		# If the cluster folder exists and the folder is numbered properly, add into the GA program
		if os.path.isdir(os.path.join(population.user_initialised_population_folder, cluster_name)) \
				and cluster_name.isdigit():
			...
			print('Check the folder in ' + str(population.user_initialised_population_folder) +
	...
	if cluster_names == []:
		print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
		print('There are no clusters in the Initialised Population in the path: ' +
			  str(population.user_initialised_population_folder))
		print('Check the Initialised Population folder ' + str(population.user_initialised_population_folder))
		print('This program will end without doing anything.')
		exit()
		...
		print('Check the cluster folders that are in the directory ' +
			  str(population.user_initialised_population_folder))
		print('Folder names in ' + str(population.user_initialised_population_folder) +
			  ' folder: ' + str(cluster_names))
		print('This program will end without doing anything.')
		exit()
	elif not is_list_sequential(cluster_names):
		print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
		print('The names of the clusters/folders in the population folder "' +
			  str(population.user_initialised_population_folder) +
			  '" must be named by numbers in sequential order starting from "1"')
		print('Check the cluster folders that are in the directory ' +
			  str(population.user_initialised_population_folder))
		print('Folder names in ' + str(population.user_initialised_population_folder) + ' folder: ' +
			  str(cluster_names))
		print('This program will end without doing anything.')
		exit()
	...
	for cluster_name in cluster_names:
		...
		if not cluster_name > 0:
			...
			print('Cluster is located in ' + str(population.user_initialised_population_folder +
												 '/' + str(cluster_name)))
			...
			toStringError = '******************************************************************\n'
			toStringError += 'There are more clusters in ' + str(population.user_initialised_population_folder) + \
							 ' than allowed in this population.\n'
			toStringError += 'The population size for this genetic algorithm run is: ' + str(population.size) + '\n'
			toStringError += 'What we are going to do is not import any of the other clusters in the population, ' \
							 'only keeping the following clusters\n'
		...
		##########################################
		print('*****************************')
		xyz_files = [file for file in os.listdir(os.path.join(population.user_initialised_population_folder,
		...
		if len(xyz_files) == 0:
			print('Check cluster '+str(cluster_name)+'. There is no .xyz file. '
													 'The prorgram requires a .xyz file of this cluster so it knows '
													 'the positions of the atoms in the cluster.')
			exit()
		...
		# overall. Take the optimised cluster obtained during the initialised_population.py program as a test to make
		# sure that the unoptimised cluster will not explode before it is used in the GA program. If you want to, you
		# can change this to use the "_Opt.xyz" file, and it probably wont be an issue. You may have to do some
		# changing of the program, but im sure it will be fine to do.

		for xyz_file in xyz_files:
			if str(cluster_name)+'_UnOpt.xyz' == xyz_file:
				break
			elif str(cluster_name) + '_Opt.xyz' == xyz_file:
				break
		else:
			print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
			print('There is no .xyz file in Cluster (folder) ' + str(cluster_name) +
				  ' called " ' + str(cluster_name) + '_Opt.xyz.')
			print('This is the cluster xyz file that is used by the genetic algorithm to represent this cluster, '
				  'as it is assume that it is that cluster, and the "_Opt" '
				  'indicates that it has been locally optimised.')
			print('Check folder ' + str(cluster_name) + ' in ' +
				  str(population.user_initialised_population_folder) + ' for the file "' +
				  str(str(cluster_name) + '_Opt.xyz') + '".')
			print('Ending the genetic algorithm without doing anything.')
			exit()
		# Performed all checks, will now add cluster
		print('Reading Cluster ' + str(cluster_name) + '...')
		UnOpt_Cluster = ase_read(population.user_initialised_population_folder + '/' +
		...
		Opt_Cluster, converged, opt_information = Initial_Energy_Function(UnOpt_Cluster.copy(),population.name,cluster_name) # note, self.name is the population name
		sys.stdout = stdout
		opt_information['output.txt'] = output.getvalue()
		if Exploded(Opt_Cluster, max_distance_between_atoms=r_ij):
			# make sure the randomised cluster has not split up when optimised.
			print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
			print("The optimised Cluster exploded. Check out why cluster " + str(cluster_name) +
				  ' has exploded by looking at cluster ' + str(cluster_name) + 's AfterOpt.traj file.')
			print('The program will now show the optimised and the unoptimised versions of cluster ' +
				  str(cluster_name))
			...
			print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
			print("This Cluster did not converge when optimised. Check out why cluster " + str(cluster_name) +
				  ' did not converge by at the following output from the stdout.')
			print('*******************************************************')
			print(opt_information['output.txt'])
			print('*******************************************************')
			# print('The program will also show the optimised and the unoptimised
			# versions of cluster ' + str(cluster_name))
			# from ase.visualize import view; view(UnOpt_Cluster); view(Opt_Cluster)
			print('This program will end without doing anything.')
			exit()
			...
			print("Cluster " + str(cluster_name) + ' in the population is too similar to one of the clusters '
												   'in your memory operator database.')
		...
		# Set some of the details about the cluster that are needed for the population collection
		# to save data to the population database.
		...
		print('Adding Cluster ' + str(cluster_name) + ' into the population of the GA.')
		population.add(index, initialised_cluster)  # Place the initialised cluster into the population

	print('*****************************')
	fitness_operator.assign_initial_population_fitnesses()
	clusters_to_create, removed_clusters_report = Check_Population_against_predation_operator(population,
																							  predation_operator)
	if not len(clusters_to_create) == 0: #len(clusters_removed_from_pop):
		print('Error in def Place_Already_Created_Clusters_In_Population of class Population, in Population.py.')
		print('Some of the clusters that you have placed in the population folder violate the Diversity operator: ' +
...
def get_tasks(population, clusters_to_create, cell_length, vacuum_to_add_length, cluster_makeup, surface, r_ij,
	...
	:param surface: This is the surface that the cluster is placed on. None means there is no surface.
	:type  surface: Organisms.GA.Surface
	:param r_ij: The maximum distance that should be between atoms to be considered bonded. This value should be
	as large a possible, to reflected the longest bond possible between atoms in the cluster.
	:type  r_ij: float
	:param rounding_criteria: The number of decimal places to round the energy of clusters to.
	:type  rounding_criteria: int
	:param Minimisation_Function: The minimisation function
...
def create_a_cluster(input_data):
	...
	while True:  # While required for self.Diversity_switch == "CNA_Full" option and Exploded(Opt_Cluster) method
		try:
			...
			output = StringIO()
			sys.stdout = output
			Opt_Cluster, converged, opt_information = Minimisation_Function(UnOpt_Cluster.deepcopy(),
																			population_name, cluster_name)
			...
			print('------------------------------------------')
			continue
		if Exploded(Opt_Cluster, max_distance_between_atoms=r_ij):  # make sure the randomised cluster has not split
			print("Cluster exploded. Will obtain a new cluster " + str(cluster_name))
			continue  # Since the cluster has exploded, we throw it out and repeat
		elif not converged:
			print("Cluster did not converge. Will obtain a new cluster " + str(cluster_name))
			continue  # Cluster did not converge, try again
		elif memory_operator.is_similar_cluster_in_memory_operator(Opt_Cluster):
			print("Cluster is too similar to existing one, will obtain a new cluster " + str(cluster_name))
			continue
		else:
			break  # break while when Explosion criteria and self.Diversity_switch is not == to "CNA_Full" are met
...
def Initialise_Population_with_Randomly_Generated_Clusters(population, cluster_makeup, surface,
	...
	clusters_to_create = list(zip(range(population.size), range(previous_cluster_name + 1,
																previous_cluster_name+population.size + 1)))
	# This part is to prevent the population making new clusters that override the clusters
	# that the user placed into the original population.
	current_cluster_names = population.get_cluster_names()  # The current Dir tag of the cluster being
	# inputted or created in the GA. Initially use to record all cluster Dir in run folder. Will be turned into
	# an int once all original clusters in the population have been recorded into the GA program.
		...
		if no_of_cpus == 1:
			...
			for task in tasks:
				try:
					...
					print('Error in Initialise_Population_with_Randomly_Generated_Clusters, Initialise_Population.py')
...
def Initialise_Population(population, cluster_makeup, surface, Minimisation_Function, Initial_Energy_Function,
	...
	This method is used when the population is first created when the genetic algorithm is just starting and
	when an epoch method is resetting the population with a new population of randomly generated clusters.
	...
	:type  generation: int
	:param get_already_created_clusters: Are there clusters that the user created in the population. True if yes,
	False if no. Default: True
	:type  get_already_created_clusters: bool.
	:param is_epoch: Has the genetic algorithm just epoched. Default: False
	:type  is_epoch: bool.
	:param epoch_due_to_population_energy_convergence: Did the genetic algorithm epochbecause the energies of the
	clusters in the last populatino converge. Default: None
	...
	"""
	print("Initalising a Starting Population of Clusters if required to take "
		  "pool up to a size of " + str(population.size))
	if get_already_created_clusters:
		print("Pool size to begin = " + str(population.get_pool_folder_size(
			population.user_initialised_population_folder)))
		previous_cluster_name = Place_Already_Created_Clusters_In_Population(population, cluster_makeup,
																			 Minimisation_Function,
																			 Initial_Energy_Function,
																			 vacuum_to_add_length, r_ij,
																			 rounding_criteria, surface,
																			 memory_operator, predation_operator,
																			 fitness_operator, previous_cluster_name)
	if len(population) < population.size:
		print('Will now population the next ' +
			  str(population.size - len(population)) + ' with randomly generated clusters.')
		previous_cluster_name = Initialise_Population_with_Randomly_Generated_Clusters(population, cluster_makeup,
																					   surface, Minimisation_Function,
																					   cell_length,
																					   vacuum_to_add_length, r_ij,
																					   rounding_criteria, no_of_cpus,
																					   memory_operator,
																					   predation_operator,
																					   fitness_operator,
																					   previous_cluster_name,
	...
	population.add_to_history_file(generation, is_epoch=is_epoch,
