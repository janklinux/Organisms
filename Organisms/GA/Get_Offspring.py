...
def Create_An_Unoptimised_Offspring(cluster_number, chance_of_mutation, cell_length, vacuum_to_add_length, population,
    ...
    Inputs:
        ...
        chance_of_mutation (float): the change than a mutation will occur, either as well as or instead of crossover,
        depending on input for creating_offspring_mode.
        cell_length (float): The size of the unit cell.
        vacuum_to_add_length (float)" The amount of vacuum to add to the unit cell"
        population (Organisms.GA.Population): This is the population that clusters will be taken from for
        mating and mutation schemes.
        creating_offspring_mode (str.): This tag indicates how the offspring are created.

            * creating_offspring_mode == 'Mating_and_Mutation_Together' - The mating scheme is run first, followed
            by a mutation scheme with some probability..
            * creating_offspring_mode == 'Either_Mating_and_Mutation' - Either the mating 'or' the mutation
            scheme will run, depending on a probability value.
    ...
    if creating_offspring_mode == 'Mating_and_Mutation_Together':
        ...
        if Will_Mutation_Occur(choice_mate_or_mutate, chance_of_mutation):
            ...
            offspring, mutation_method = mutation_procedure.run(cluster_to_mutate=cluster_to_mutate,
    ...
    #     print('The offspring contains ' + str(len(offspring)) +
    #     ' atoms, but should contain ' + str(len(population[0])))
    #     print('Check this')
...
def Will_Mutation_Occur(choice_mate_or_mutate, chance_of_mutation):
    ...
    :param choice_mate_or_mutate: This is a random value between 0 and 1. If this value is less than
    self.chance_of_mutation, this indicates to perform a mutation
...
def Create_An_Offspring(input_data):
    ...
    :returns: The offspring and a string containing any information that would be useful for the user to see
    about how this cluster was made
    ...
    while True:
        ...
        # elements = ''
        # for k, v in UnOpt_offspring.get_elemental_makeup().items():
        #     elements += str(k) + str(v)
        # print('Relaxing {}...'.format(elements))
        Opt_offspring = None
        while True:
            try:
            ...
            if try_statement_counter == 20:
                ...
                print('If you see this message multiple times during a genetic algorithm run, '
                      'this is probably of some concern and it may pay to follow this up.', file=sys.stderr)
                break
        did_explode = Exploded(Opt_offspring, max_distance_between_atoms=r_ij)
        if not converged:
            # toString += "Cluster did not converge. " \
            #             "Will disregard this cluster and try mating or mutation method again\n"
        ...
        if no_of_explosions + no_of_not_converged == 10:
            ...
            # print('If this is happening very rarely, this is fine and is not of major concern.', file=sys.stderr)
            # print(
            #     'If you see this message multiple times during a genetic algorithm run, '
            #     'it is probably of some concern and it may pay to follow this up.', file=sys.stderr)
            break
